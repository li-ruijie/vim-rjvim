...
EXPORT BOOL
SetVimForeground(HWND hHWND) {
  DWORD dwThreadIDFG = GetWindowThreadProcessId(GetForegroundWindow(), NULL);
  DWORD dwThreadIDCU = GetCurrentThreadId();
  AttachThreadInput(dwThreadIDFG, dwThreadIDCU, TRUE);
  SetForegroundWindow(hHWND);
  AttachThreadInput(dwThreadIDFG, dwThreadIDCU, FALSE);
  if (GetForegroundWindow() == hHWND) {
    return FALSE;
  } else {
    return TRUE;
  }
...
SetAlpha(LONG nTrans) {
  ...
  if (hDllUser32) {
    ...
    if (pfnSetLayeredWindowAttributes) {
      ...
      if (hTop) {
        if (nTrans == 255) {
          SetWindowLongPtr(hTop, GWL_EXSTYLE,
              GetWindowLongPtr(hTop, GWL_EXSTYLE) & ~WS_EX_LAYERED);
        } else {
          SetWindowLongPtr(hTop, GWL_EXSTYLE,
              GetWindowLongPtr(hTop, GWL_EXSTYLE) | WS_EX_LAYERED);
...
EnableCaption(LONG bCaption) {
  ...
  if (hTop = GetVimWindow()) {
    if (bCaption == 0)
      SetWindowLongPtr(hTop, GWL_STYLE,
          GetWindowLongPtr(hTop, GWL_STYLE) & ~WS_CAPTION);
    else
      SetWindowLongPtr(hTop, GWL_STYLE,
          GetWindowLongPtr(hTop, GWL_STYLE) | WS_CAPTION);
  }
