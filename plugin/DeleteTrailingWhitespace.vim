if !exists('g:loaded_DeleteTrailingWhitespace')
    ...
    augroup DeleteTrailingWhitespace
        autocmd!
        autocmd BufWritePre *
            \ try call DeleteTrailingWhitespace#InterceptWrite()                             |
            \     catch /^DeleteTrailingWhitespace:/                                         |
            \     echoerr substitute(v:exception, '^\CDeleteTrailingWhitespace:\s*', '', '') |
            \ endtry
    augroup END
    " commands
    function! s:Before()
    ...
    command! -bar -range=%
        \ DeleteTrailingWhitespace call <SID>Before() <Bar>
        \ call setline(<line1>, getline(<line1>))<Bar>
        \ call <SID>After()<Bar>
        \ call DeleteTrailingWhitespace#Delete(<line1>, <line2>)
endif
